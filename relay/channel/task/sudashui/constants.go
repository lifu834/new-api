...
// ⚠️ **本站的 4K/2K 档没有接进海外组，是有意的**：
// sdas-gf-seedance-2.0-4k(¥0.35/秒) 与 -2k 都标注"**超分模型**"——低分辨率
// 生成后放大，而 dimensio / mai 的 4K 是原生。把它映射到同一个对外名
// (sd-2.0-4k)，会让付 4K 价的客户在故障切换时随机拿到超分货，正是本项目
// 一直在防的"同名不同能力"。宁可 4K 只有两路，也不要三路但质量不一致。
// 若将来实测证明其超分质量可接受，再补映射。
