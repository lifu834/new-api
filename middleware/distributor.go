...
func getModelRequest(c *gin.Context) (*ModelRequest, bool, error) {
	...
	} else if strings.Contains(c.Request.URL.Path, "/v1/images/async") {
		// Async image-generation task channel (chatgpt2api): POST submit / GET fetch-by-id.
		// Platform is derived from the selected channel type (numeric) in RelayTaskSubmit,
		// so we only need to resolve the model (submit) and mark fetch as channel-less.
		relayMode := relayconstant.RelayModeUnknown
		if c.Request.Method == http.MethodPost {
			req, err := getModelFromRequest(c)
			if err != nil {
